...
class UserBasedCF(object):
    ...
    def loadfile(filename):
        ...
        for i, line in enumerate(fp):
            '''
            yield语句生成generator,带有yiled的函数会被视为一个generator。
            在for循环执行时,执行到yield时,函数就返回一个迭代值；下次迭代时，代码从yiled的下一句继续执行。
            '''
            yield line.strip('\r\n')
            # if i % 100000 == 0:
            if i / 100000 == 1:
    ...
    def generate_dataset(self, filename, pivot=0.7):
        ...
        for line in self.loadfile(filename):
            ...
            if random.random() < pivot:
                self.trainset.setdefault(user, {})      # setdefault:如果字典中包含有给定键，则返回该键对应的值，否则返回为该键设置的值。
    ...
    def calc_user_sim(self):
        ...
        for user, movies in self.trainset.items():  # dict.items(): 遍历(key,value)元组数组
            for movie in movies:
